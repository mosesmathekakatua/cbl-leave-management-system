...
const LoginView: React.FC<{ onRegister: () => void, onForgot: () => void }> = ({ onRegister, onForgot }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const user = state.users.find(u => u.name.toLowerCase() === normalizedName);
    
    if (!user) {
      setError("Registry Error: Profile not found.");
      return;
    }
    if (user.isBlocked) {
    ...
    if (!user.isApproved) {
      setError("Awaiting Administrative Authorization.");
      return;
    }
    ...
    if (user.pin === pin) {
      ...
      setError("Authentication Failure: PIN Incorrect.");
    }
  };
        ...
        <div className="p-12 text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-6 text-left">
            <div>
              ...
              <input required className="w-full font-bold" placeholder="Enter Official Name" value={name} onChange={e => setName(e.target.value)} />
            </div>
            <div>
...
const RegistrationView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      alert("Registration submitted to the Cloud. Awaiting manager approval.");
      onBack();
    }, 1000);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#EAE8DA] p-6">
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-12 border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="button" onClick={onBack} className="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-black transition-colors">Back to Login</button>
        </form>
      </div>
    </div>
  );
